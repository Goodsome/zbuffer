...
norm = np.linalg.norm(normal, 2, axis=1).reshape(-1, 1)
normal /= norm
normal[normal[:, 2] < 0] *= -1
normal.shape = [-1, 1, 3]
...
tri_v = np.concatenate((tri_v, dx, solve_v, normal, colors), axis=1)

print(tri_v)

for i in range(int(max(vertex[:, 1])), -1, -1):
    tran = (tri_v[:, 0, 1] >= i) * (tri_v[:, 0, 1] < i + 1)
    act_v = np.append(act_v, tri_v[tran]).reshape(-1, 7, 3)
    act_v = act_v[act_v[:, 2, 1] < i]
    if np.size(act_v) == 0:
        continue
    act_v1 = act_v[act_v[:, 1, 1] < i]
    act_v2 = act_v[act_v[:, 1, 1] >= i]
    act_v = np.concatenate((act_v2, act_v1), axis=0)
